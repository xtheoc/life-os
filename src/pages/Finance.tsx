...
export default function Finance() {
  ...
  function clearAllImports() {
    financeImports.forEach(imp => dispatch({ type: 'DELETE_FINANCE_IMPORT', payload: { id: imp.id } }))
    setConfirmClearAll(false)
    toast('All finance data cleared', 'success')
  }
          ...
          {financeImports.length > 0 && (
            confirmClearAll ? (
              <div className="flex items-center gap-1.5">
                <span className="text-xs text-danger font-display">Clear all data?</span>
                <button onClick={clearAllImports} className="px-2 py-1 rounded text-xs bg-danger/20 text-danger hover:bg-danger/30 transition-colors">Yes</button>
                <button onClick={() => setConfirmClearAll(false)} className="px-2 py-1 rounded text-xs bg-white/5 text-muted hover:bg-white/10 transition-colors">No</button>
              </div>
            ) : (
              <button onClick={() => setConfirmClearAll(true)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 text-muted hover:text-danger hover:bg-danger/10 text-sm transition-colors">
